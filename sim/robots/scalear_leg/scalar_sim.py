...
class pyb_sim(object):
    ...
    def __init__(self, urdf_filename, bodyFixed = True, DoFnum=3, RobotStartPos = [0,0,0.5], RobotStartOrientation = [0,0,0], delta_t = None):
        ...
        if DoFnum == 3:
            ...
            self.HOME_POSE = [0,0,np.pi/2, 0,0,np.pi/2, 0,0,np.pi/2, 0,0,np.pi/2]
        elif DoFnum == 6:
            ...
            self.HOME_POSE = [0,0,np.pi/2,0,0,0,  0,0,np.pi/2,0,0,0,   0,0,np.pi/2,0,0,0,   0,0,np.pi/2,0,0,0]
        else:
    ...
    def connectLinks(self):
        for i in range(self.N_leg):
            cid = p.createConstraint(self.RobotId, self.linkNameToId[self.connectedLinkNameList[i][0]], 
                               self.RobotId, self.linkNameToId[self.connectedLinkNameList[i][1]], p.JOINT_POINT2POINT, [0, 0, 0], [0, 0, 0], [0, 0, 0])
            p.changeConstraint(cid, maxForce=self.maxForce*1000)

    def setPassiveJoints(self):
        for i in range(self.passiveJointNum):
            p.setJointMotorControl2(bodyUniqueId=self.RobotId, jointIndex=self.jointNameToId[self.passiveJointNameList[i]], controlMode=p.VELOCITY_CONTROL, targetVelocity=0, force=self.passiveJointFriction)
    ...
    def movetoPose(self, targetPose):
        ...
        for i in range(self.N_leg):
            newtargetPose[i*self.DoFnum+2] = newtargetPose[i*self.DoFnum+2] - np.pi/2
        p.setJointMotorControlArray(self.RobotId, self.MotorIdList, p.POSITION_CONTROL, newtargetPose, forces=[self.maxForce] * self.motorJointNum)
    ...
    def reset(self):
        ...
        self.RobotId = p.loadURDF(self.urdf_filename, self.RobotStartPos, self.RobotStartOrientation,useFixedBase = self.bodyFixed)
